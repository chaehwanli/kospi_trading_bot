...
6. 백테스트 결과에 대상 주식 가격이 백테스트 기간 중 우상향(Uptrend), 우하향(Downtrend), 박스권 횡보(Sideways) 중이었는지 판단하는 로직을 추가한다.
   - **판단 로직**: 기간 내 주가(Close)에 대해 선형 회귀(Linear Regression)를 수행하여 기울기(Slope)를 계산한다.
   - **Trend 기준** (Threshold: 0.00005, 봉 하나당 약 0.005% 변동):
     - `UPTREND`: Slope > 0.00005
     - `DOWNTREND`: Slope < -0.00005
     - `SIDEWAYS`: -0.00005 <= Slope <= 0.00005
